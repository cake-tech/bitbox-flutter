...
EncodedPushData encode(Uint8List buffer, number, offset) {
  ...
  if (size == 1) {
    ...
    buffer.buffer.asByteData().setUint8(offset, Opcodes.OP_PUSHDATA1);
    buffer.buffer.asByteData().setUint8(offset + 1, number);

    // 16 bit
  } else if (size == 3) {
    buffer.buffer.asByteData().setUint8(offset, Opcodes.OP_PUSHDATA2);
    buffer.buffer.asByteData().setUint16(offset + 1, number, Endian.little);

    // 32 bit
  } else {
    buffer.buffer.asByteData().setUint8(offset, Opcodes.OP_PUSHDATA4);
